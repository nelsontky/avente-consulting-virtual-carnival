import "phaser";

export function getOverWorld(map: Phaser.Tilemaps.Tilemap) {
  return [
    map.addTilesetImage("Pandamaru Circus 2", "Pandamaru Circus 2"),
    map.addTilesetImage("Pandamaru Circus 1", "Pandamaru Circus 1"),
    map.addTilesetImage("Pandamaru Merry-Go-Round", "Pandamaru Merry-Go-Round"),
    map.addTilesetImage("Pandamaru Market 1", "Pandamaru Market 1"),
    map.addTilesetImage("Pandamaru Magic Tent", "Pandamaru Magic Tent"),
    map.addTilesetImage("Emerald (Tommy)", "Emerald (Tommy)"),
    map.addTilesetImage("Outside (Hoenn) (Tommy)", "Outside (Hoenn) (Tommy)"),
    map.addTilesetImage("Pandamaru Egypt", "Pandamaru Egypt"),
    map.addTilesetImage("carmin sur mer", "carmin sur mer"),
    map.addTilesetImage("Pandamaru Port Props", "Pandamaru Port Props"),
    map.addTilesetImage("Pandamaru Goods 1", "Pandamaru Goods 1"),
    map.addTilesetImage("Brown cave sand", "Brown cave sand"),
    map.addTilesetImage("Dirt cave highlight", "Dirt cave highlight"),
    map.addTilesetImage(
      "celianna_templetiles_torches",
      "celianna_templetiles_torches"
    ),
    map.addTilesetImage("TileC Celianna", "TileC Celianna"),
    map.addTilesetImage("pandamaru tree 1", "pandamaru tree 1"),
    map.addTilesetImage("pandamaru tree 2", "pandamaru tree 2"),
    map.addTilesetImage(
      "pokemon_tileset_from_public_tiles_by_chaoticcherrycake_d5xdb0y-pre",
      "pokemon_tileset_from_public_tiles_by_chaoticcherrycake_d5xdb0y-pre"
    ),
    map.addTilesetImage("pandamaru tree 3", "pandamaru tree 3"),
    map.addTilesetImage("pandamaru tree 4", "pandamaru tree 4"),
    map.addTilesetImage("pandamaru tree 5", "pandamaru tree 5"),
    map.addTilesetImage("pandamaru tree goods", "pandamaru tree goods"),
    map.addTilesetImage("Pandamaru Playground", "Pandamaru Playground"),
    map.addTilesetImage("Pandamaru Enclosure 2", "Pandamaru Enclosure 2"),
    map.addTilesetImage("Pandamaru Train", "Pandamaru Train"),
    map.addTilesetImage("Pandamaru Train Wagon", "Pandamaru Train Wagon"),
    map.addTilesetImage("Hoenn Shipp", "Hoenn Shipp"),
    map.addTilesetImage("Hoeen (1) (Tommy)", "Hoeen (1) (Tommy)"),
    map.addTilesetImage("Boats (Tommy)", "Boats (Tommy)"),
    map.addTilesetImage("062-CF_Lava01", "062-CF_Lava01"),
    map.addTilesetImage("Tentages (Tommy)", "Tentages (Tommy)"),
    map.addTilesetImage("Flooring", "Flooring"),
    map.addTilesetImage("Grey Oval Tracks (Tommy)", "Grey Oval Tracks (Tommy)"),
    map.addTilesetImage("aiseh1", "aiseh1"),
    map.addTilesetImage("Pandamaru Retro floor", "Pandamaru Retro floor"),
    map.addTilesetImage("fairytale hero", "fairytale hero"),
    map.addTilesetImage("cherry_blossom_trees_1", "cherry_blossom_trees_1"),
    map.addTilesetImage("amap", "amap"),
    map.addTilesetImage("Cherrytree", "Cherrytree"),
    map.addTilesetImage("boxy_bold_font", "boxy_bold_font"),
    map.addTilesetImage("Scifi 2", "Scifi 2"),
    map.addTilesetImage("Scifi 1", "Scifi 1"),
    map.addTilesetImage("Scifi 3", "Scifi 3"),
    map.addTilesetImage("modern_cyberpunk_cyanide", "modern_cyberpunk_cyanide"),
    map.addTilesetImage("Space", "Space"),
    map.addTilesetImage("Space 2", "Space 2"),
    map.addTilesetImage("Pandamaru Fruits", "Pandamaru Fruits"),
    map.addTilesetImage("YouTrip", "YouTrip"),
    map.addTilesetImage("ezbuy", "ezbuy"),
    map.addTilesetImage("avalon", "avalon"),
    map.addTilesetImage("lim kee pau", "lim kee pau"),
    map.addTilesetImage("Station1", "Station1"),
    map.addTilesetImage("Station5", "Station5"),
    map.addTilesetImage("Station2", "Station2"),
    map.addTilesetImage("Station4", "Station4"),
    map.addTilesetImage("grass", "grass"),
    map.addTilesetImage("Station3", "Station3"),
    map.addTilesetImage("LOGO", "LOGO"),
    map.addTilesetImage("A Dark", "A Dark"),
    map.addTilesetImage("Zoom In Out", "Zoom In Out"),
  ];
}

export function getRoom0(map: Phaser.Tilemaps.Tilemap) {
  return [
    map.addTilesetImage("Scifi 2", "Scifi 2"),
    map.addTilesetImage("Scifi 1", "Scifi 1"),
    map.addTilesetImage("Space 2", "Space 2"),
    map.addTilesetImage("futuristic", "futuristic"),
    map.addTilesetImage("Emerald (Tommy)", "Emerald (Tommy)"),
    map.addTilesetImage("Pandamaru Enclosure 2", "Pandamaru Enclosure 2"),
  ];
}

export function getRoom1(map: Phaser.Tilemaps.Tilemap) {
  return [
    map.addTilesetImage("egypt", "egypt"),
    map.addTilesetImage("Pandamaru Egypt", "Pandamaru Egypt"),
    map.addTilesetImage(
      "celianna_templetiles_torches",
      "celianna_templetiles_torches"
    ),
    map.addTilesetImage("Emerald (Tommy)", "Emerald (Tommy)"),
    map.addTilesetImage("Pandamaru Fruits", "Pandamaru Fruits"),
  ];
}

export function getRoom2(map: Phaser.Tilemaps.Tilemap) {
  return [
    map.addTilesetImage("treetop", "treetop"),
    map.addTilesetImage("Space", "Space"),
    map.addTilesetImage("treetop sofa", "treetop sofa"),
    map.addTilesetImage("Emerald (Tommy)", "Emerald (Tommy)"),
    map.addTilesetImage("Pandamaru Goods 1", "Pandamaru Goods 1"),
  ];
}

export function getRoom3(map: Phaser.Tilemaps.Tilemap) {
  return [
    map.addTilesetImage("Carnival Room", "Carnival Room"),
    map.addTilesetImage("Pandamaru Enclosure 2", "Pandamaru Enclosure 2"),
    map.addTilesetImage("Emerald (Tommy)", "Emerald (Tommy)"),
    map.addTilesetImage("Pandamaru Magic Tent", "Pandamaru Magic Tent"),
  ];
}

export function getRoom4(map: Phaser.Tilemaps.Tilemap) {
  return [
    map.addTilesetImage("Boss Room", "Boss Room"),
    map.addTilesetImage("Emerald (Tommy)", "Emerald (Tommy)"),
    map.addTilesetImage("Pandamaru Merry-Go-Round", "Pandamaru Merry-Go-Round"),
    map.addTilesetImage("Pandamaru Circus 2", "Pandamaru Circus 2"),
  ];
}
